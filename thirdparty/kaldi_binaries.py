...
included_libraries = {
    ...
    "darwin": [
        "libfst.10.dylib",
        "libfstfarscript.10.dylib",
        "libfstscript.10.dylib",
        "libfstfar.10.dylib",
        "libfstngram.10.dylib",
        "libkaldi-nnet.dylib",
        "libkaldi-hmm.dylib",
        "libkaldi-util.dylib",
        "libkaldi-thread.dylib",
        "libkaldi-base.dylib",
        "libkaldi-tree.dylib",
        "libkaldi-matrix.dylib",
        "libkaldi-feat.dylib",
        "libkaldi-transform.dylib",
        "libkaldi-lm.dylib",
        "libkaldi-gmm.dylib",
        "libkaldi-lat.dylib",
        "libkaldi-decoder.dylib",
        "libkaldi-fstext.dylib",
        "libkaldi-rnnlm.dylib",
        "libkaldi-nnet2.dylib",
        "libkaldi-nnet3.dylib",
        "libkaldi-cudamatrix.dylib",
...
def collect_linux_tools_binaries(directory):
    ...
    for name in os.listdir(bin_dir):
        ...
        if value == exe_ext and key in included_filenames:
            ...
            if os.path.exists(out_path) and os.path.getsize(in_path) > os.path.getsize(
                out_path
            ):
    ...
    for root, dirs, files in os.walk(src_dir, followlinks=True):
        ...
        for name in files:
            ...
            if sys.platform == "darwin":
                ...
                for l in libs:
                    if (
                        l.startswith("/usr") and not l.startswith("/usr/local")
                    ) or l.startswith("/System"):
                        continue
                    lib = os.path.basename(l)
                    subprocess.call(
                        [
