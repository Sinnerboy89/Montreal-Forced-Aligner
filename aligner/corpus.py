...
def extract_temp_channels(wav_path, temp_directory):
    ...
    if not os.path.exists(A_path):
        with wave.open(wav_path, "rb") as inf, wave.open(A_path, "wb") as af, wave.open(
            B_path, "wb"
        ) as bf:
...
class Corpus(object):
    ...
    def __init__(
        ...
        if not os.path.isdir(directory):
            raise CorpusError(
                "The specified path for the corpus ({}) is not a directory.".format(
                    directory
                )
            )
        if num_jobs < 1:
        ...
        for root, dirs, files in os.walk(self.directory, followlinks=True):
            for f in sorted(files):
                ...
                if lab_name is not None:
                    ...
                    self.file_directory_mapping[utt_name] = (
                        root.replace(self.directory, "").lstrip("/").lstrip("\\")
                    )
                    ...
                    if n_channels == 2:
                        ...
                        A_path, B_path = extract_temp_channels(
                            wav_path, self.temp_directory
                        )
                    elif n_channels > 2:
                    ...
                    for i, ti in enumerate(tg.tiers):
                        ...
                        for interval in ti:
                            ...
                            if not words:
                                continue
                            begin, end = (
                                round(interval.minTime, 4),
                                round(interval.maxTime, 4),
                            )
                            utt_name = "{}_{}_{}_{}".format(
                                speaker_name, file_name, begin, end
                            )
                            utt_name = (
                                utt_name.strip().replace(" ", "_").replace(".", "_")
                            )
                            if n_channels == 1:
                                if (
                                    self.feat_mapping
                                    and utt_name not in self.feat_mapping
                                ):
                                    self.ignored_utterances.append(utt_name)
                                self.segments[utt_name] = "{} {} {}".format(
                                    file_name, begin, end
                                )
                            ...
                            self.speak_utt_mapping[speaker_name].append(utt_name)
                    if n_channels == 2:
                        self.file_directory_mapping[A_name] = (
                            root.replace(self.directory, "").lstrip("/").lstrip("\\")
                        )
                        self.file_directory_mapping[B_name] = (
                            root.replace(self.directory, "").lstrip("/").lstrip("\\")
                        )
                    else:
                        self.file_directory_mapping[file_name] = (
                            root.replace(self.directory, "").lstrip("/").lstrip("\\")
                        )
                    self.tg_count += 1
        ...
        self.issues_check = (
            self.ignored_utterances
            or self.no_transcription_files
            or self.textgrid_read_errors
            or self.unsupported_sample_rate
            or self.decode_error_files
        )
        ...
        if bad_speakers:
            msg = (
                "The following speakers had multiple speaking rates: {}. "
                "Please make sure that each speaker has a consistent sampling rate.".format(
                    ", ".join(bad_speakers)
                )
            )
        ...
        if self.num_jobs < len(self.sample_rates.keys()):
            ...
            msg = (
                "The number of jobs was set to {}, due to the different sample rates in the dataset. "
    ...
    def find_best_groupings(self):
        ...
        while remaining_jobs > 0:
            ...
            addable = sorted(
                [k for k, v in jobs_per_sample_rate.items() if v == min_num],
        ...
        for k, v in jobs_per_sample_rate.items():
            ...
            configs = [
                (
                    job_num + x,
                    {"sample-frequency": k, "low-freq": 20, "high-freq": 7800},
                )
                for x in range(v)
            ]
            ind = 0
    ...
    def speaker_utterance_info(self):
        ...
        average_utterances = (
            sum(len(x) for x in self.speak_utt_mapping.values()) / num_speakers
        )
        msg = "Number of speakers in corpus: {}, average number of utterances per speaker: {}".format(
            num_speakers, average_utterances
        )
        root_logger = logging.getLogger()
    ...
    def grouped_utt2fst(self, dictionary, num_frequent_words=10):
        ...
        most_frequent = sorted(word_frequencies.items(), key=lambda x: -x[1])[
            :num_frequent_words
        ]
    ...
    def _split_utt2fst(self, directory, dictionary):
        pattern = "utt2fst.{}"
        save_groups(
            self.grouped_utt2fst(dictionary), directory, pattern, multiline=True
        )
    ...
    def combine_feats(self):
        ...
        if len(self.feat_mapping.keys()) != len(self.utt_speak_mapping.keys()):
            ...
            for k, v in self.speak_utt_mapping.items():
                self.speak_utt_mapping[k] = list(
                    filter(lambda x: x in self.feat_mapping, v)
                )
    ...
    def get_feat_dim(self, feature_config):
        ...
        path = os.path.join(
            self.split_directory(), feature_config.feature_id + ".0.scp"
        )
        with open(os.devnull, "w") as devnull:
            dim_proc = subprocess.Popen(
                [thirdparty_binary("feat-to-dim"), "scp:" + path, "-"],
    ...
    def figure_utterance_lengths(self):
        ...
        if os.path.exists(feat_path):
            with open(os.devnull, "w") as devnull:
                dim_proc = subprocess.Popen(
                    [thirdparty_binary("feat-to-len"), "scp:" + feat_path, "ark,t:-"],
                    stdout=subprocess.PIPE,
                    stderr=devnull,
                )
                stdout, stderr = dim_proc.communicate()
    ...
    def create_subset(self, subset, feature_config):
        ...
        if larger_subset_num < self.num_utterances:
            utts = sorted(
                (
                    x
                    for x in self.utterance_lengths.keys()
                    if " " in self.text_mapping[x]
                ),
        ...
        split_directory = self.split_directory()
        subset_directory = os.path.join(
            self.output_directory, "subset_{}".format(subset)
        )
        ...
        for j in range(self.num_jobs):
            for fn in ["text.{}", "text.{}.int", "utt2spk.{}"]:
                with open(
                    os.path.join(split_directory, fn.format(j)), "r", encoding="utf8"
                ) as inf, open(
                    os.path.join(subset_directory, fn.format(j)), "w", encoding="utf8"
                ) as outf:
                    for line in inf:
                        s = line.split()
                        ...
                        outf.write(line)
            with open(
                os.path.join(split_directory, "spk2utt.{}".format(j)),
                "r",
                encoding="utf8",
            ) as inf, open(
                os.path.join(subset_directory, "spk2utt.{}".format(j)),
                "w",
                encoding="utf8",
            ) as outf:
                for line in inf:
            ...
            if feature_config is not None:
                base_path = os.path.join(
                    split_directory, feature_config.feature_id + ".{}.scp".format(j)
                )
                subset_scp = os.path.join(
                    subset_directory, feature_config.feature_id + ".{}.scp".format(j)
                )
