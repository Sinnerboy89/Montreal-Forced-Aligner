...
class Dictionary(object):
    ...
    topo_sil_template = (
        "<State> {cur_state} <PdfClass> {cur_state} {transitions} </State>"
    )
    ...
    def __init__(
            ...
            for i, line in enumerate(inf):
                ...
                if not line:
                    raise DictionaryError(
                        "Line {} of {} does not have a pronunciation.".format(
                            i, input_path
                        )
                    )
        ...
        if not self.graphemes:
            raise DictionaryFileError(
                "No words were found in the dictionary path {}".format(input_path)
            )
    ...
    def save_oovs_found(self, directory):
        ...
        """
        with open(
            os.path.join(directory, "oovs_found.txt"), "w", encoding="utf8"
        ) as f, open(
            os.path.join(directory, "oov_counts.txt"), "w", encoding="utf8"
        ) as cf:
            for oov in sorted(
                self.oovs_found.keys(), key=lambda x: (-self.oovs_found[x], x)
            ):
                f.write(oov + "\n")
    ...
    def silence_csl(self):
        ...
        if self.position_dependent_phones:
            return ":".join(
                map(str, (self.phone_mapping[x] for x in self.positional_sil_phones))
            )
        else:
    ...
    def _write_word_boundaries(self):
        boundary_path = os.path.join(
            self.output_directory, "phones", "word_boundary.txt"
        )
        boundary_int_path = os.path.join(
            self.output_directory, "phones", "word_boundary.int"
        )
        with open(boundary_path, "w", encoding="utf8") as f, open(
            ...
            if self.position_dependent_phones:
                for p in sorted(
                    self.phone_mapping.keys(), key=lambda x: self.phone_mapping[x]
                ):
                    if p == "<eps>":
                        continue
                    cat = "nonword"
    ...
    def _write_topo(self):
        ...
        with open(filepath, "w") as f:
            ...
            for i in range(self.num_sil_states):
                ...
                states.append(
                    self.topo_sil_template.format(cur_state=i, transitions=transition)
                )
            f.write("\n".join(states))
    ...
    def _write_phone_sets(self):
        ...
        with open(sets_file, "w", encoding="utf8") as setf, open(
            ...
            for i, sp in enumerate(self.sil_phones):
                ...
                setintf.write(
                    " ".join(map(str, (self.phone_mapping[x] for x in mapped))) + "\n"
                )
                if i == 0:
    ...
    def _write_extra_questions(self):
        ...
        with open(phone_extra, "w", encoding="utf8") as outf, open(
            ...
            intf.write(
                " ".join(map(str, (self.phone_mapping[x] for x in nonsils))) + "\n"
            )
            if self.position_dependent_phones:
                for p in self.positions:
                    ...
                    intf.write(
                        " ".join(map(str, (self.phone_mapping[x] for x in line))) + "\n"
                    )
                for p in [""] + self.positions:
    ...
    def _write_fst_binary(self, disambig=False):
        if disambig:
            lexicon_fst_path = os.path.join(
                self.output_directory, "lexicon_disambig.text.fst"
            )
        ...
        if self.debug:
            ...
            with open(log_path, "w") as logf:
                ...
                dot_proc = subprocess.Popen(
                    [thirdparty_binary("dot"), "-Tpdf", "-O", dot_path], stderr=logf
                )
    ...
    def _write_fst_text(self, disambig=False):
        ...
        with open(lexicon_fst_path, "w", encoding="utf8") as outf:
            if self.sil_prob != 0:
                outf.write(
                    "\t".join(
                        map(str, [startstate, loopstate, "<eps>", "<eps>", nosilcost])
                    )
                    + "\n"
                )
                ...
                outf.write(
                    "\t".join(
                        map(str, [startstate, loopstate, nonoptsil, "<eps>", silcost])
                    )
                    + "\n"
                )
                outf.write(
            ...
            for w in sorted(self.words.keys()):
                for phones, prob, disambig_symbol in sorted(self.words[w]):
                    ...
                    while len(phones) > 0:
                        p = phones.pop(0)
                        if len(phones) > 0 or (
                            disambig and disambig_symbol is not None
                        ):
                            ns = nextstate
                            nextstate += 1
                            outf.write(
                            ...
                            outf.write(
                                "\t".join(
                                    map(
                                        str,
                                        [s, loopstate, p, word_or_eps, local_nosilcost],
                                    )
                                )
                                + "\n"
                            )
                            outf.write(
                                "\t".join(
                                    map(
                                        str,
                                        [s, silstate, p, word_or_eps, local_silcost],
                                    )
                                )
                                + "\n"
                            )
