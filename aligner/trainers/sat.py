    ...
    def init_training(
        ...
        with open(log_path, "w") as logf:
            subprocess.call(
                ...
                + context_opts
                + [treeacc_path, sets_int_path, questions_path],
                stderr=logf,
            )

        with open(extra_question_int_path, "r") as inf, open(
            questions_path, "a"
        ) as outf:
            for line in inf:
                outf.write(line)
        ...
        with open(log_path, "w") as logf:
            subprocess.call(
                ...
                + [
                    "--verbose=1",
                    "--max-leaves={}".format(self.initial_gaussians),
        ...
        compile_train_graphs(
            self.train_directory,
            self.dictionary.output_directory,
            self.data_directory,
            self.corpus.num_jobs,
        )
        ...
        if os.path.exists(os.path.join(align_directory, "trans.0")):
            for i in range(self.corpus.num_jobs):
                shutil.copy(
                    os.path.join(align_directory, "trans.{}".format(i)),
