...
class BaseTrainer(object):
    ...
    def gaussian_increment(self):
        return int(
            (self.max_gaussians - self.initial_gaussians)
            / self.final_gaussian_iteration
        )
    ...
    def _setup_for_init(self, identifier, temporary_directory, corpus, dictionary):
        ...
        if self.subset is not None:
            self.data_directory = corpus.subset_directory(
                self.subset, self.feature_config
            )

    def init_training(
        self, identifier, temporary_directory, corpus, dictionary, previous_trainer
    ):
        raise NotImplementedError
    ...
    def parse_log_directory(self, directory, iteration, num_jobs, call_back):
        ...
        skipped_transition_regex = re.compile(
            r"(\d+) out of (\d+) transition-states skipped due to insufficient data"
        )

        log_like_regex = re.compile(
            r"Overall avg like per frame = ([-0-9.]+|nan) over (\d+) frames"
        )
        error_files = []
    ...
    def get_unaligned_utterances(self):
        ...
        for i in range(self.corpus.num_jobs):
            path = os.path.join(
                self.align_directory, "log", "align.final.{}.log".format(i)
            )
            if not os.path.exists(path):
                continue
            with open(path, "r") as f:
    ...
    def train(self, call_back=None):
        ...
        for i in iters:
            ...
            if i in self.realignment_iterations:
                ...
                if self.debug:
                    compute_alignment_improvement(
                        i, self, self.train_directory, self.corpus.num_jobs
                    )

            acc_stats(
                i, self.train_directory, self.data_directory, self.corpus.num_jobs, self
            )
            log_path = os.path.join(self.log_directory, "update.{}.log".format(i))
            with open(log_path, "w") as logf:
                acc_files = [
                    os.path.join(self.train_directory, "{}.{}.acc".format(i, x))
                ...
                est_proc = subprocess.Popen(
                    [
                        ...
                        "--mix-up=" + str(num_gauss),
                        "--power=" + str(self.power),
                        model_path,
                        "{} - {}|".format(
                            thirdparty_binary("gmm-sum-accs"),
            ...
            if not os.path.exists(next_model_path):
                raise (
                    Exception(
                        "There was an error training in iteration {}, please check the logs.".format(
                            i
                        )
                    )
                )
            self.parse_log_directory(
                self.log_directory, i, self.corpus.num_jobs, call_back
            )
            if i < self.final_gaussian_iteration:
                num_gauss += self.gaussian_increment
        shutil.copy(
            os.path.join(self.train_directory, "{}.mdl".format(self.num_iterations)),
            final_mdl_path,
        )
        shutil.copy(
            os.path.join(self.train_directory, "{}.occs".format(self.num_iterations)),
            os.path.join(self.train_directory, "final.occs"),
        )
        if not self.debug:
    ...
    def export_textgrids(self):
        ...
        convert_ali_to_textgrids(
            self,
            os.path.join(self.align_directory, "textgrids"),
            self.align_directory,
            self.dictionary,
            self.corpus,
            self.corpus.num_jobs,
