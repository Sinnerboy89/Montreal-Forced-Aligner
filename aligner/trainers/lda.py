        ...
        if self.data_directory != self.corpus.split_directory():
            ...
            for j in range(self.corpus.num_jobs):
                base_path = os.path.join(
                    corpus.split_directory(),
                    self.feature_config.feature_id + ".{}.scp".format(j),
                )
                ...
                with open(subset_scp, "w") as f:
                    for line in filtered:
                        f.write(line.strip() + "\n")
        super(LdaTrainer, self).init_training(
            identifier, temporary_directory, corpus, dictionary, previous_trainer
        )
