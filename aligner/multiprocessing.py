...
def acc_stats_func(directory, iteration, job_name, feat_path):  # pragma: no cover
    log_path = os.path.join(
        directory, "log", "acc.{}.{}.log".format(iteration, job_name)
    )
...
def acc_stats(iteration, directory, split_directory, num_jobs, config):
    ...
    jobs = [
        [directory, iteration, x, os.path.join(split_directory, feat_name.format(x))]
...
def parse_transitions(path, phones_path):
    ...
    cur_phone = None
    current = 0
    with open(path, encoding="utf8") as f, open(
...
def compile_train_graphs_func(
    directory, lang_directory, split_directory, job_name, debug=True
):  # pragma: no cover
    ...
    if debug:
        with open(log_path, "w", encoding="utf8") as logf:
            with open(transition_path, "w", encoding="utf8") as f:
                subprocess.call(
                    [thirdparty_binary("show-transitions"), phones_file_path, mdl_path],
                    stdout=f,
                    stderr=logf,
                )
            parse_transitions(transition_path, triphones_file_path)
    log_path = os.path.join(
        directory, "log", "compile-graphs.0.{}.log".format(job_name)
    )
    ...
    with open(
        os.path.join(split_directory, "text.{}.int".format(job_name)),
        "r",
        encoding="utf8",
    ) as inf, open(fst_path, "wb") as outf, open(
        log_path, "w", encoding="utf8"
    ) as logf:
    ...
    if debug:
        utterances = []
        with open(
            os.path.join(split_directory, "utt2spk.{}".format(job_name)),
            "r",
            encoding="utf8",
        ) as f:
            for line in f:
        ...
        with open(log_path, "a", encoding="utf8") as logf:
            ...
            proc = subprocess.Popen(
                [
                    thirdparty_binary("fstcopy"),
                    "ark:{}".format(fst_path),
                    "ark,scp:{},{}".format(fst_ark_path, fst_scp_path),
                ],
                stderr=logf,
            )
            proc.communicate()
                ...
                for line in f:
                    ...
                    try:
                        dot_proc = subprocess.Popen(
                            [thirdparty_binary("dot"), "-Tpdf", "-O", dot_path],
                            stderr=logf,
                        )
...
def compile_train_graphs(
    directory, lang_directory, split_directory, num_jobs, debug=False
):
    """
...
def mono_align_equal_func(
    mono_directory, split_directory, job_name, feat_path
):  # pragma: no cover
...
def mono_align_equal(mono_directory, split_directory, num_jobs, config):
    ...
    jobs = [
        [
            ...
            x,
            os.path.join(
                split_directory, config.feature_file_base_name + ".{}.scp".format(x)
            ),
        ]
        for x in range(num_jobs)
    ]
...
def align_func(
    directory, iteration, job_name, mdl, config, feat_path, output_directory
):  # pragma: no cover
    fst_path = os.path.join(directory, "fsts.{}".format(job_name))
    log_path = os.path.join(
        output_directory, "log", "align.{}.{}.log".format(iteration, job_name)
    )
...
def align_no_pool(
    ...
    mdl = "{} --boost={} {} {} - |".format(
        ...
        config.boost_silence,
        optional_silence,
        make_path_safe(mdl_path),
    )
...
def align(
    ...
    for i in range(num_jobs):
        log_path = os.path.join(
            output_directory, "log", "align.{}.{}.log".format(iteration, i)
        )
...
def compile_information_func(log_directory, corpus, job_num):
    ...
    features_path = os.path.join(
        corpus.split_directory(), "log", "make_mfcc.{}.log".format(job_num)
    )
...
def compute_alignment_improvement_func(iteration, config, model_directory, job_name):
    ...
    log_path = os.path.join(
        model_directory, "log", "get_ctm.{}.{}.log".format(iteration, job_name)
    )
    ...
    phone_ctm_path = os.path.join(
        model_directory, "phone.{}.{}.ctm".format(iteration, job_name)
    )
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        ...
        align_proc = subprocess.Popen(
            [
                ...
                os.path.join(config.dictionary.phones_dir, "word_boundary.int"),
                model_path,
                "ark:-",
                "ark:-",
            ],
...
def compute_alignment_improvement(iteration, config, model_directory, num_jobs):
    ...
    for job in jobs:
        compute_alignment_improvement_func(*job)

    # with mp.Pool(processes=num_jobs, initializer=init, initargs=(child_env,)) as pool:
    #     r = False
    #     try:
    #         results = [
    #             pool.apply_async(compute_alignment_improvement_func, args=i)
    #             for i in jobs
    #         ]
    #         output = [p.get() for p in results]
    #     except OSError as e:
    ...
    try:
        previous_alignments = parse_iteration_alignments(
            model_directory, previous_iteration, num_jobs
        )
    except FileNotFoundError:
        return
    current_alignments = parse_iteration_alignments(
        model_directory, iteration, num_jobs
    )
    utterance_aligned_diff, mean_difference = compare_alignments(
        previous_alignments, current_alignments, config.feature_config.frame_shift
    )
    ...
    if iteration in config.realignment_iterations:
        with open(alignment_diff_path, "a", encoding="utf8") as f:
            f.write(
                "{},{},{},{},{}\n".format(
                    iteration,
                    len(current_alignments),
                    len(previous_alignments),
                    utterance_aligned_diff,
                    mean_difference,
                )
            )
    if not config.debug:
        for j in range(num_jobs):
            phone_ctm_path = os.path.join(
                model_directory, "phone.{}.{}.ctm".format(previous_iteration, j)
            )
            os.remove(phone_ctm_path)


def ali_to_textgrid_func(
    align_config, model_directory, dictionary, corpus, job_name
):  # pragma: no cover
    text_int_path = os.path.join(
        corpus.split_directory(), "text.{}.int".format(job_name)
    )
    log_path = os.path.join(
        model_directory, "log", "get_ctm_align.{}.log".format(job_name)
    )
    ali_path = os.path.join(model_directory, "ali.{}".format(job_name))
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        ...
        align_proc = subprocess.Popen(
            [
                thirdparty_binary("lattice-align-words"),
                os.path.join(dictionary.phones_dir, "word_boundary.int"),
                model_path,
                "ark:-",
                "ark,t:" + aligned_path,
            ],
...
def convert_ali_to_textgrids(
    align_config, output_directory, model_directory, dictionary, corpus, num_jobs
):
    """
    Multiprocessing function that aligns based on the current model
...
def tree_stats_func(
    directory, ci_phones, mdl, feat_path, ali_path, job_name
):  # pragma: no cover
    context_opts = []
...
def tree_stats(
    directory, align_directory, split_directory, ci_phones, num_jobs, config
):
    """
    ...
    jobs = [
        [
            ...
            mdl_path,
            os.path.join(split_directory, feat_name.format(x)),
            os.path.join(align_directory, "ali.{}".format(x)),
            x,
        ]
        for x in range(num_jobs)
    ]
    ...
    tree_accs = [
        os.path.join(directory, "{}.treeacc".format(x)) for x in range(num_jobs)
    ]
    log_path = os.path.join(directory, "log", "sum_tree_acc.log")
    with open(log_path, "w", encoding="utf8") as logf:
        subprocess.call(
            [thirdparty_binary("sum-tree-stats"), os.path.join(directory, "treeacc")]
            + tree_accs,
            stderr=logf,
        )
    # for f in tree_accs:
...
def calc_fmllr_func(
    ...
    log_path = os.path.join(
        directory, "log", "fmllr.{}.{}.log".format(model_name, job_name)
    )
    ali_path = os.path.join(directory, "ali.{}".format(job_name))
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        subprocess.call(
            [thirdparty_binary("ali-to-post"), "ark:" + ali_path, "ark:" + post_path],
            stderr=logf,
        )
        ...
        utt2spk_path = os.path.join(
            config.corpus.split_directory(), "utt2spk.{}".format(job_name)
        )
        base_scp = os.path.join(config.corpus.split_directory(), base_scp)
        feat_fmllr_scp_path = os.path.join(
            config.corpus.split_directory(),
            config.feature_config.feature_id + ".{}.scp".format(job_name),
        )
        feat_fmllr_ark_path = os.path.join(
            config.corpus.split_directory(),
            config.feature_config.feature_id + ".{}.ark".format(job_name),
        )
        subprocess.call(
            [
                thirdparty_binary("transform-feats"),
...
def lda_acc_stats_func(directory, split_dir, align_directory, config, ci_phones, i):
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        spliced_feat_path = os.path.join(
            split_dir, config.feature_config.feature_id + ".{}.scp".format(i)
        )
...
def lda_acc_stats(directory, split_dir, align_directory, config, ci_phones, num_jobs):
    ...
    shutil.copyfile(
        os.path.join(directory, "lda.mat"),
        os.path.join(config.corpus.split_directory(), "lda.mat"),
    )
...
def calc_lda_mllt_func(
    ...
    log_path = os.path.join(
        directory, "log", "lda_mllt.{}.{}.log".format(model_name, job_name)
    )
    ali_path = os.path.join(directory, "ali.{}".format(job_name))
    if not initial:
        mdl_path = os.path.join(directory, "{}.mdl".format(model_name))
    ...
    feat_path = os.path.join(
        split_directory, config.feature_config.feature_id + ".{}.scp".format(job_name)
    )
    post_path = os.path.join(directory, "post.{}".format(job_name))
    weight_path = os.path.join(directory, "weight.{}".format(job_name))
...
def calc_lda_mllt(
    ...
    mdl_path = os.path.join(directory, "{}.mdl".format(model_name))
    log_path = os.path.join(
        directory, "log", "transform_means.{}.log".format(model_name)
    )
    ...
    with open(log_path, "a", encoding="utf8") as logf:
        ...
        for x in range(num_jobs):
            macc_list.append(
                os.path.join(directory, "{}.{}.macc".format(model_name, x))
            )
        subprocess.call(
            [thirdparty_binary("est-mllt"), new_mat_path] + macc_list, stderr=logf
        )
        subprocess.call(
            [
...
def gmm_gselect_func(config, x):
    ...
    feat_path = os.path.join(
        config.data_directory, config.feature_file_base_name + ".{}.scp".format(x)
    )
    with open(log_path, "w", encoding="utf8") as logf:
...
def acc_global_stats_func(config, x, iteration):
    log_path = os.path.join(
        config.train_directory, "log", "acc.{}.{}.log".format(iteration, x)
    )
    feat_path = os.path.join(
        config.data_directory, config.feature_file_base_name + ".{}.scp".format(x)
    )
    with open(log_path, "w", encoding="utf8") as logf:
        ...
        gmm_global_acc_proc = subprocess.Popen(
            [
                ...
                "--gselect="
                + "ark:"
                + os.path.join(config.train_directory, "gselect.{}".format(x)),
...
def acc_ivector_stats_func(config, x, iteration):
    ...
    feat_path = os.path.join(
        config.data_directory, config.feature_config.feature_id + ".{}.scp".format(x)
    )
    with open(log_path, "w", encoding="utf8") as logf:
        subsample_feats_proc = subprocess.Popen(
            [
        ...
        acc_stats_proc = subprocess.Popen(
            [
                ...
                "ark:" + os.path.join(config.train_directory, "post.{}".format(x)),
                os.path.join(
                    config.train_directory, "accinit.{}.{}".format(iteration, x)
                ),
            ],
            stdin=subsample_feats_proc.stdout,
            stderr=logf,
...
def acc_ivector_stats(config, num_jobs, iteration):
    ...
    accinits = [
        os.path.join(config.train_directory, "accinit.{}.{}".format(iteration, j))
        for j in range(num_jobs)
    ]
    log_path = os.path.join(
        config.train_directory, "log", "sum_acc.{}.log".format(iteration)
    )
    with open(log_path, "w", encoding="utf8") as logf:
...
def extract_ivectors_func(config, x):
    ...
    log_path = os.path.join(
        config.align_directory, "log", "extract_ivectors.{}.log".format(x)
    )
    feat_name = config.feature_file_base_name
    feat_name += ".{}.scp"
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        ali_to_post_proc = subprocess.Popen(
            [thirdparty_binary("ali-to-post"), "ark:" + ali_path, "ark:-"],
            stderr=logf,
            stdout=subprocess.PIPE,
        )
        ...
        post_to_weight_proc = subprocess.Popen(
            [thirdparty_binary("post-to-weights"), "ark:-", "ark:" + weight_path],
            stderr=logf,
        ...
        with open(os.devnull, "w", encoding="utf8") as devnull:
            dim_proc = subprocess.Popen(
                [thirdparty_binary("feat-to-dim"), "scp:" + feat_scp_path, "-"],
                stdout=subprocess.PIPE,
                stderr=devnull,
            )
        ...
        ivector_ark_path = os.path.join(
            config.data_directory, "ivector.{}.ark".format(x)
        )
        ivector_scp_path = os.path.join(
            config.data_directory, "ivector.{}.scp".format(x)
        )
        append_proc = subprocess.Popen(
            [
                thirdparty_binary("append-vector-to-feats"),
                "scp:" + feat_scp_path,
                "ark:" + ivectors_path,
                "ark:-",
            ],
        ...
        select_proc = subprocess.Popen(
            [
                thirdparty_binary("select-feats"),
...
def get_egs_func(config, align_directory, valid_uttlist, train_subset_uttlist, x):
    ...
    log_path = os.path.join(
        config.train_directory, "log", "get_egs_feats.{}.log".format(x)
    )
    with open(log_path, "w", encoding="utf8") as logf:
        # Gets "feats" (Kaldi)
        ...
        base_feature_scp = os.path.join(
            config.data_directory, config.feature_file_base_name + ".{}.scp".format(x)
        )
        filtered = filter_scp(
            train_subset_uttlist, os.path.join(config.data_directory, base_feature_scp)
        )
        training_features_path = os.path.join(
            config.train_directory, "features_for_nnet_train.{}.scp".format(x)
        )
        ...
        filtered = filter_scp(
            valid_uttlist, os.path.join(config.train_directory, base_feature_scp)
        )
        validation_features_path = os.path.join(
            config.train_directory, "features_for_nnet_valid.{}.scp".format(x)
        )
    ...
    log_path = os.path.join(
        config.train_directory, "log", "ali_to_post.{}.log".format(x)
    )
    with open(log_path, "w", encoding="utf8") as logf:
        ali_to_pdf_proc = subprocess.Popen(
            [
                thirdparty_binary("ali-to-pdf"),
                os.path.join(align_directory, "final.mdl"),
        ...
        ali_to_post_proc = subprocess.Popen(
            [thirdparty_binary("ali-to-post"), "ark:-", "ark:-"],
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        nnet_get_egs_proc = subprocess.Popen(
            [thirdparty_binary("nnet-get-egs")]
            + ivectors_opt
            + [
        ...
        nnet_copy_egs_proc = subprocess.Popen(
            [
                thirdparty_binary("nnet-copy-egs"),
                "ark:-",
    ...
    log_path = os.path.join(
        config.train_directory, "log", "nnet_shuffle_egs.{}.log".format(x)
    )
    with open(log_path, "w", encoding="utf8") as logf:
        nnet_copy_egs_proc = subprocess.Popen(
            [
        ...
        nnet_shuffle_egs_proc = subprocess.Popen(
            [
                thirdparty_binary("nnet-shuffle-egs"),
                "--srand=" + str(x),
                "ark:-",
...
def get_egs(config, ali_dir, valid_uttlist, train_subset_uttlist):
    """
...
def get_lda_nnet_func(config, align_directory, x):
    ...
    base_feature_scp = os.path.join(
        ...
        config.feature_file_base_name.replace("_ivectors", "") + ".{}.scp".format(x),
    )
    spliced_feature_ark = os.path.join(
        config.data_directory, "features_for_nnet_lda.{}.ark".format(x)
    )
    spliced_feature_scp = os.path.join(
        config.data_directory, "features_for_nnet_lda.{}.scp".format(x)
    )
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        if config.feature_config.ivectors:
            splice_proc = subprocess.Popen(
                [
                    ...
                    "--left-context={}".format(
                        config.feature_config.splice_left_context
                    ),
                    "--right-context={}".format(
                        config.feature_config.splice_right_context
                    ),
            ...
            with open(ivector_dim_path, "w", encoding="utf8") as outf:
                dim_proc = subprocess.Popen(
                    [thirdparty_binary("feat-to-dim"), "scp:" + ivector_scp_path, "-"],
                    stderr=logf,
                    stdout=outf,
                )
        else:
        ...
        with open(feat_dim_path, "w", encoding="utf8") as outf:
            dim_proc = subprocess.Popen(
                [thirdparty_binary("feat-to-dim"), "scp:" + base_feature_scp, "-"],
                stderr=logf,
                stdout=outf,
            )
...
def get_lda_nnet(config, align_directory, num_jobs):
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        ...
        sum_lda_accs_proc = subprocess.Popen(
            [
                thirdparty_binary("sum-lda-accs"),
        ...
        lda_mat_proc = subprocess.Popen(
            [
                thirdparty_binary("nnet-get-feature-transform"),
...
def nnet_train_trans(nnet_dir, align_dir, prev_ali_path, num_jobs):
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        train_trans_proc = subprocess.Popen(
            [
                thirdparty_binary("nnet-train-transitions"),
...
def nnet_train_func(nnet_dir, egs_dir, mdl, i, x):
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        ...
        train_proc = subprocess.Popen(
            [
                thirdparty_binary("nnet-train-parallel"),
...
def nnet_align_func(i, config, train_directory, align_directory, x):
    ...
    ivector_scp_path = os.path.join(config.data_directory, "ivector.{}.scp".format(x))
    with open(log_path, "w", encoding="utf8") as logf, open(ali_path, "wb") as outf:
        paste_proc = subprocess.Popen(
            [
                thirdparty_binary("paste-feats"),
                "scp:" + feat_path,
        ...
        align_proc = subprocess.Popen(
            [
                thirdparty_binary("nnet-align-compiled"),
                "--beam={}".format(config.beam),
                "--retry-beam={}".format(config.retry_beam),
                mdl_path,
                "ark:" + fst_path,
...
def compute_prob_func(i, nnet_dir, egs_dir, model_path, x):
    log_path = os.path.join(
        nnet_dir, "log", "compute_prob_train.{}.{}.log".format(i, x)
    )
    with open(log_path, "w", encoding="utf8") as logf:
        compute_prob_proc = subprocess.Popen(
            [
                thirdparty_binary("nnet-compute-prob"),
                model_path,
...
def ali_to_textgrid_kaldi_func(
    ali_directory,
    model_directory,
    lang_dir,
    split_data_dir,
    ...
    with open(log_path, "w", encoding="utf8") as logf, open(
        text_int_path, "w", encoding="utf8"
    ) as outf:
        sym2int_proc = subprocess.Popen(
            [
                sym2int_script,
                "--map-oov",
                oov,
    ...
    frame_shift = 10 / 1000
    log_path = os.path.join(
        ali_directory, "log", "get_ctm_align.{}.log".format(job_name)
    )
    with open(log_path, "w", encoding="utf8") as logf:
        lin_proc = subprocess.Popen(
            [
        ...
        align_proc = subprocess.Popen(
            [
                thirdparty_binary("lattice-align-words"),
                os.path.join(phones_dir, "word_boundary.int"),
                model_path,
                "ark:-",
                "ark:" + aligned_path,
            ],
...
def convert_ali_to_textgrids_kaldi(
    ali_directory,
    model_directory,
    lang_dir,
    split_data_dir,
    ...
    for job in jobs:
        ali_to_textgrid_kaldi_func(*job)

    # with mp.Pool(processes=num_jobs, initializer=init, initargs=(child_env,)) as pool:
    #     r = False
    #     try:
    #         results = [
    #             pool.apply_async(ali_to_textgrid_kaldi_func, args=i) for i in jobs
    #         ]
    #         output = [p.get() for p in results]
    #     except OSError as e:
    #         if hasattr(e, "errno") and e.errorno == 24:
...
def get_average_posteriors_func(i, nnet_dir, prev_egs_dir, config, x):
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        nnet_to_raw_nnet_proc = subprocess.Popen(
            [
                thirdparty_binary("nnet-to-raw-nnet"),
        ...
        nnet_subset_egs_proc = subprocess.Popen(
            [
                thirdparty_binary("nnet-subset-egs"),
        ...
        nnet_compute_from_egs_proc = subprocess.Popen(
            [
                thirdparty_binary("nnet-compute-from-egs"),
                os.path.join(nnet_dir, "{}_raw.mdl".format(i)),
                "ark:-",
                "ark:-",
            ],
        ...
        matrix_sum_rows_proc = subprocess.Popen(
            [thirdparty_binary("matrix-sum-rows"), "ark:-", "ark:-"],
            stdin=nnet_compute_from_egs_proc.stdout,
            stdout=subprocess.PIPE,
            stderr=logf,
        )
        vector_sum_proc = subprocess.Popen(
            [
...
def get_average_posteriors(i, nnet_dir, prev_egs_dir, config, num_jobs):
    ...
    for job in jobs:
        get_average_posteriors_func(*job)
...
def relabel_egs_func(i, nnet_dir, egs_in, alignments, egs_out, x):
    ...
    with open(log_path, "w", encoding="utf8") as logf:
        ali_to_pdf_proc = subprocess.Popen(
            [thirdparty_binary("ali-to-pdf"), model_path, "ark:" + alignments, "ark:-"],
            stdout=subprocess.PIPE,
            # stdin=alignments,
            stderr=logf,
        )
        nnet_relabel_egs_proc = subprocess.Popen(
            [
                thirdparty_binary("nnet-relabel-egs"),
                "ark:-",
