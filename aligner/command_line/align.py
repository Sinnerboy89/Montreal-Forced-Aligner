...
def align_included_model(args):
    ...
    args.acoustic_model_path = os.path.join(
        pretrained_dir,
        "{}{}".format(args.acoustic_model_path.lower(), AcousticModel.extension),
    )
    align_corpus(args)


def validate_args(args):
    if args.acoustic_model_path.lower().endswith(AcousticModel.extension):
        align_corpus(args)
    else:
        raise Exception("Acoustic model not found; check args")
...
if __name__ == "__main__":  # pragma: no cover
    ...
    parser.add_argument(
        "acoustic_model_path",
        help="Full path to the archive containing pre-trained model",
    ),
    parser.add_argument(
        "output_directory",
    ...
    parser.add_argument(
        "-v",
        "--verbose",
        help="Print more information during alignment",
        action="store_true",
    )
    parser.add_argument(
        "-c", "--clean", help="Remove files from previous runs", action="store_true"
    )
    parser.add_argument(
        "-d",
        "--debug",
